...
def parse_args():
    ...
    parser.add_argument("--target-hit-rates", type=float, nargs="+",
                        default=[0.1, 0.3, 0.5, 0.7, 0.5, 0.3],
...
def main():
    ...
    results = generate_batch(multi_qas, port)
    ...
    # Get final cache metrics from server (FYI)
    cached_tokens, total_tokens, hit_rate = get_cache_hit_rate_from_metrics(port)
    if total_tokens > 0:
        print_highlight(f"== Server-reported cache hit rate: {hit_rate:.4f} ==")
    ...
    with open(args.output_file, "w") as f:
        ...
        if total_tokens > 0:
            f.write(f"Server cache hit rate: {hit_rate:.4f}\n")
